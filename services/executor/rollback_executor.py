...
def run_execution(
    store: ControlPlaneStore, gateway: AwsGateway, *, operation_id: str, worker_id: str | None = None
) -> None:
    ...
    store.append_event(
        record.incident_id, EventType.EXECUTION_STARTED.value, {"operation_id": operation_id, "plan_id": plan.plan_id}
    )
    ...
    try:
        ...
        if plan.replay_requests:
            ...
            store.update_operation(
                operation_id,
                lambda op: op.model_copy(update={"phase": OperationPhase.REPLAY, "updated_at": datetime.now(UTC)}),
            )
            apply_transition(store, record.incident_id, IncidentState.REPLAYING)
            replay_outcomes = run_replay(
                store,
                gateway,
                app_config,
                plan,
        ...
        store.append_event(
            record.incident_id,
            EventType.RECEIPT_CREATED.value,
            {"receipt_id": receipt.receipt_id, "outcome": outcome.value},
        )
        store.update_operation(
            operation_id,
            lambda op: op.model_copy(
...
def _apply_rollback(
    ...
    store.update_operation(
        operation_id, lambda op: op.model_copy(update={"phase": OperationPhase.APPLY, "updated_at": datetime.now(UTC)})
    )
    apply_transition(store, incident_id, IncidentState.APPLYING)

    try:
        new_state = gateway.update_alias(
            app_config.function_name, plan.alias_name, plan.expected_alias_revision_id, plan.to_version
        )
        ...
        if current.current_version == plan.to_version:
            ...
            raise ExecutionAborted(
                "EXECUTION_AMBIGUOUS", f"UpdateAlias result unknown and alias not at target: {exc}"
            ) from exc
    ...
    store.update_execution_record(
        ...
        lambda rec: rec.model_copy(
            update={
                "actual_alias_revision_id": new_state.alias_revision_id,
                "applied_version": new_state.current_version,
            }
        ),
    )
    store.append_event(
        incident_id,
        EventType.EXECUTION_RECONCILED.value,
        {
            "operation_id": operation_id,
            "applied_version": new_state.current_version,
            "alias_revision_id": new_state.alias_revision_id,
        },
    )
...
def _verify_recovery(
    ...
    store.update_operation(
        operation_id, lambda op: op.model_copy(update={"phase": OperationPhase.VERIFY, "updated_at": datetime.now(UTC)})
    )
    ...
    canary_result = gateway.invoke(
        app_config.function_name, plan.alias_name, {"request_id": canary_id, "document": CANARY_DOCUMENT}
    )
    invocation_ok = canary_result.function_error is None and canary_result.executed_version == plan.to_version
    checks.append(
        VerificationCheck(
            check="canary_invocation",
    ...
    checks.append(
        VerificationCheck(
            check="canary_result_and_hash",
            passed=result_ok,
            observed_at=datetime.now(UTC),
            details=(
                f"result found, hash={result_record.payload_sha256}" if result_record else "no result record found"
            ),
        )
    )
    return checks
...
def _unresolved_request_ids(
    store: ControlPlaneStore, demo_run_id: str, replay_outcomes: list[RequestExecutionOutcome]
) -> list[str]:
    resolved_ids = {
        o.request_id
        for o in replay_outcomes
        if o.outcome in (ReceiptRequestOutcome.COMPLETED, ReceiptRequestOutcome.ALREADY_COMPLETED)
    }
...
def _build_receipt(
    store, incident_id, plan, approval, operation_id, checks, replay_outcomes, unresolved, outcome
) -> Receipt:
    ...
    for r in requests:
        ...
        if r.status == RequestStatus.FAILED:
            request_lines.append(
                RequestExecutionOutcome(request_id=r.request_id, outcome=ReceiptRequestOutcome.NOT_REPLAYED)
            )
        elif r.status == RequestStatus.COMPLETED:
            request_lines.append(
                RequestExecutionOutcome(
                    request_id=r.request_id, outcome=ReceiptRequestOutcome.ALREADY_COMPLETED, result_id=r.result_id
                )
            )
...
def _mark_needs_attention(
    store: ControlPlaneStore, operation_id: str, incident_id: str, message: str, *, error_code: str = "NEEDS_ATTENTION"
) -> None:
    store.append_event(
        incident_id,
        EventType.INCIDENT_NEEDS_ATTENTION.value,
        {"operation_id": operation_id, "reason": message, "code": error_code},
    )
    store.update_operation(
        operation_id,
        lambda op: op.model_copy(
