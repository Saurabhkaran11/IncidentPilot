...
def run_replay(
    ...
    *,
    ...
    store.append_event(
        incident_id,
        EventType.REPLAY_STARTED.value,
        {
            "operation_id": operation_id,
            "request_count": len(plan.replay_requests),
        },
    )
    ...
    for ref in plan.replay_requests:
        if datetime.now(UTC) > deadline:
            logger.warning(
                "replay %s: deadline exceeded, stopping (processed %d/%d)",
                operation_id,
                len(outcomes),
                len(plan.replay_requests),
            )
            break
        ...
        if current.current_version != plan.to_version:
            logger.warning(
                "replay %s: alias moved to %s (expected %s), stopping",
                operation_id,
        ...
        if demo_request is None or demo_request.payload_sha256 != ref.payload_sha256:
            outcomes.append(
                RequestExecutionOutcome(
                    ...
                    outcome=ReceiptRequestOutcome.CONFLICT,
                    details="request not found or payload changed since plan was built",
                )
            )
            continue
        ...
        if invoke_result.function_error is not None:
            outcome = RequestExecutionOutcome(
                request_id=ref.request_id,
                outcome=ReceiptRequestOutcome.STILL_FAILED,
                details=invoke_result.error_message,
            )
            store.update_demo_request(
                ref.request_id,
                lambda r: r.model_copy(update={"attempts": r.attempts + 1, "updated_at": datetime.now(UTC)}),
            )
        else:
            body = invoke_result.payload or {}
            if body.get("conflict"):
                outcome = RequestExecutionOutcome(
                    request_id=ref.request_id, outcome=ReceiptRequestOutcome.CONFLICT, result_id=ref.request_id
                )
                new_status = RequestStatus.NEEDS_ATTENTION
            elif body.get("already_completed"):
                outcome = RequestExecutionOutcome(
                    request_id=ref.request_id, outcome=ReceiptRequestOutcome.ALREADY_COMPLETED, result_id=ref.request_id
                )
                new_status = RequestStatus.COMPLETED
            else:
                outcome = RequestExecutionOutcome(
                    request_id=ref.request_id, outcome=ReceiptRequestOutcome.COMPLETED, result_id=ref.request_id
                )
                new_status = RequestStatus.COMPLETED
            # Bind both loop-dependent values as defaults rather than
            # closing over them: correct today only because the store calls
            # the mutator synchronously, and that is not a property worth
            # depending on.
            store.update_demo_request(
                ref.request_id,
                lambda r, s=new_status, rid=ref.request_id: r.model_copy(
                    update={"status": s, "result_id": rid, "attempts": r.attempts + 1, "updated_at": datetime.now(UTC)}
                ),
            )
        ...
        store.append_event(
            incident_id,
            EventType.REQUEST_RECONCILED.value,
            {
                "operation_id": operation_id,
                "request_id": ref.request_id,
    ...
    for ref in plan.replay_requests:
        if ref.request_id not in attempted_ids:
            outcomes.append(
                RequestExecutionOutcome(request_id=ref.request_id, outcome=ReceiptRequestOutcome.NOT_REPLAYED)
            )
