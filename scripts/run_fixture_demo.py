...
def main() -> int:
    ...
    incident = Incident(
        ...
        app_id=app_config.app_id,
        demo_run_id=run.demo_run_id,
        fingerprint=f"{app_config.app_id}:v7",
        ...
        impact=ImpactSummary(
            failed_requests=3, completed_requests=0, first_failure_at=first_failure_at, mode=Mode.FIXTURE
        ),
    )
    ...
    incident = store.update_incident(
        ...
        lambda i: i.model_copy(
            update={"state": IncidentState.INVESTIGATING, "version": i.version + 1, "updated_at": datetime.now(UTC)}
        ),
    )
    ...
    store.create_operation(
        Operation(
            operation_id=investigation_op_id,
    ...
    if incident.state != IncidentState.AWAITING_APPROVAL:
        log(
            f"incident state is {incident.state.value}, not awaiting_approval -- stopping here (this is a valid abstention outcome)."
        )
        return 0
    ...
    diff_snapshot = ReleaseDiffSnapshot(
        alias_name=diff_data["alias_name"],
        current_version=diff_data["current_version"],
        known_good_version=diff_data["known_good_version"],
        alias_revision_id=diff_data["alias_revision_id"],
        weighted_routing=diff_data["weighted_routing"],
        schema_compatible=diff_data["schema_compatible"],
        changes=diff_data["changes"],
        processor_role_matches_manifest=diff_data["processor_role_matches_manifest"],
    )
    plan = evaluate(
        incident_id=incident.incident_id,
    ...
    incident = store.update_incident(
        ...
        incident.version,
        lambda i: i.model_copy(
            update={"active_plan_id": plan.plan_id, "version": i.version + 1, "updated_at": datetime.now(UTC)}
        ),
    )
    ...
    incident = store.update_incident(
        ...
        incident.version,
        lambda i: i.model_copy(
            update={"active_operation_id": execution_op_id, "version": i.version + 1, "updated_at": datetime.now(UTC)}
        ),
    )
